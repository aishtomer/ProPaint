...
function Toolbox() {
	...
	var addToolIcon = function(icon, name) {
		...
		document.getElementById(name + "sideBarItem").setAttribute("title", name);
	};
	...
	this.selectTool = function(toolName) {
		...
		//toolName
		var allParametersList = selectAll(".parameterDivs");
		for (let i = 0; i < allParametersList.length; i++){
			allParametersList[i].style("display","none");
		}
		...
		for (var i = 0; i < this.tools.length; i++) {
			if (this.tools[i].name == toolName) {
				...
				if (this.selectedTool != null){
					var toolParameters = selectAll(this.selectedTool.className);
					for (let j = 0; j < toolParameters.length; j++){
						toolParameters[j].style("display","flex");
					}
				}
